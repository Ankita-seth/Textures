...
ConstantBufferPerObject cbPerObject; // this is the constant buffer data we will send to the gpu which will be placed in the resource we created above)
...
bool Texture::InitializeWindow(HINSTANCE hInstance,
    int ShowWnd,
    bool fullscreen)
...
bool Texture::InitD3D()
{
    ...
    for (int i = 0; i < frameBufferCount; ++i)
    {
        ...
        HeapProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
        ResourcesDesc = CD3DX12_RESOURCE_DESC::Buffer(D3D12_DEFAULT_RESOURCE_PLACEMENT_ALIGNMENT);
    ...
    BYTE* imageData;
    int imageSize = LoadImageDataFromFile(&imageData, textureDesc, L"xyz.jpg", imageBytesPerRow);
    ...
    HeapProperties = CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD);
    ResourcesDesc = CD3DX12_RESOURCE_DESC::Buffer(textureUploadBufferSize);

    // now we create an upload heap to upload our texture to the GPU
    hr = device->CreateCommittedResource(
        &HeapProperties, // upload heap
        D3D12_HEAP_FLAG_NONE, // no flags
        &ResourcesDesc, // resource description for a buffer (storing the image data in this heap just to copy to the default heap)
    ...
    barrier = CD3DX12_RESOURCE_BARRIER::Transition(textureBuffer, D3D12_RESOURCE_STATE_COPY_DEST, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
    commandList->ResourceBarrier(1, &barrier);
...
void Texture::WaitForPreviousFrame()
{
    HRESULT hr;
